...
if(!isset($_POST['filter_submit'])){
      ...
      $packaged_orders_query="SELECT * FROM ORDERS WHERE DELIVERY_STATUS='pending'  ORDER BY ORDER_DATE_FULL DESC";
      ...
      if(mysqli_num_rows($sellers_items)==0){ $small_screen_order_list='<div class="col-md-5 col-sm-5 small-screen-display">No New Orders To Deliver</div>';}
                 ...
                 $title="<h4 style='background-color:#D91E18;'>You Have ".$pending_orders." orders!</h4>";
		      	 ...
		      	 while($orders=mysqli_fetch_assoc($sellers_items)){
						 ...
						 $net_income=$cost-$diggimall_commission;
				      	 ...
				      	 $con_status_button='<button onclick="confirm_this(\''.$order_id.'\');" class="btn btn-danger"><i class="fa fa-check"></i> Confirm Payment</button>';
				      	 $cross_status_button='<button onclick="crosscheck_this(\''.$order_id.'\');" class="btn btn-danger"><i class="fa fa-check"></i> Crosschecked</button>';
				      	 $on_status_button='<button onclick="onroute_this(\''.$order_id.'\');" class="btn btn-danger"><i class="fa fa-check"></i> On route</button>';
				      	 $status_button='<button onclick="deliver_this(\''.$order_id.'\');" class="btn btn-danger"><i class="fa fa-check"></i> Delivered & Accepted</button>';
				        ...
				        //small screen
				         $small_screen_order_list.='<div class="col-md-5 col-sm-5 small-screen-display">
														...
														<p><b>Requests:</b><span><b>'.$requirements.'</b></span></p>
	    ...
	    $sellers_items=$filtering->run_admin_accounts_order_search();
...
$(function(){
    ...
    confirm_this=function(a){
    	//a - order id

    	var order_id=a;

    	  $.post("inc/category_parser.php",{conorder_id:order_id},function(data){
                      
                    $(".status"+order_id).html('Payment Confirmed');
    ...
    crosscheck_this=function(a){
    	//a - order id

    	var order_id=a;

    	  $.post("inc/category_parser.php",{crossorder_id:order_id},function(data){
                      
                    $(".status"+order_id).html('Order Crosschecked');
